...
vault_t* init_vault(uint8_t *s_pref, size_t s_pref_size, size_t b_size, size_t L, size_t C) {
    ...
    uint8_t b_random_bytes[b_size];
    int ret = mbedtls_ctr_drbg_random(&ctr_drbg, b_random_bytes,b_size );
    ...
    for (size_t b = 0; b < b_size; b++) {
        ...
        for (size_t l = 0; l < L; l++) {
            ...
            for (size_t i = 0; i < C; i++) {
                //printf("%u\n", s_pref[vault->bit_lockers[b].locks[l].indexes[i]]);
                xor_result ^= s_pref[vault->bit_lockers[b].locks[l].indexes[i]];
            }
            vault->bit_lockers[b].locks[l].lock = xor_result^b_random_bytes[b];
...
void store_vault(vault_t *vault, nvs_handle_t nvs_handle, size_t b_size, size_t L, size_t C) {
    ...
    for (size_t b = 0; b < b_size; b++) {
        for (size_t l = 0; l < L; l++) {
            ...
            err = nvs_set_blob(nvs_handle, index_key, vault->bit_lockers[b].locks[l].indexes, C * sizeof(uint8_t));
            if (err != ESP_OK) {
...
void print_vault(vault_t *vault, size_t b_size, size_t L, size_t C) {
    for (size_t b = 0; b < b_size; b++) {
        for (size_t l = 0; l < L; l++) {
            printf("Bit %d, Lock %d: Lock: %02x, Indexes: ", (uint8_t)b, (uint8_t)l, vault->bit_lockers[b].locks[l].lock);
            for (size_t i = 0; i < C; i++) {
                printf("%d", vault->bit_lockers[b].locks[l].indexes[i]);
                if (i < C - 1) {
                    printf(", ");
...
vault_t* get_vault(nvs_handle_t nvs_handle, size_t b_size, size_t L, size_t C) {
    ...
    for (size_t b = 0; b < b_size; b++) {
        ...
        for (size_t l = 0; l < L; l++) {
            ...
            size_t required_size = C * sizeof(uint8_t);
